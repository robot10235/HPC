...
void my_pthread_rwlock_rdlock(struct my_pthread_rwlock_t* rwlock) {
   ...
   if(WRI_PRIOR == 1) {
      /* write first, when writer exist, reader should sleep */
      ...
      /* if no writer exists, all readers can read */
      else {
         rwlock->read_run = rwlock->read_run + 1;         
      }
   }
      ...
      /* read first */ 
      /*  if no writer runs, readers run first */
      if(rwlock->write_run == 0) {
         rwlock->read_run = rwlock->read_run + 1;
      }
      /* if writer runs, reader should sleep and wait  */ 
      else {
         /* reader sleep */ 
         rwlock->read_wait = rwlock->read_wait + 1;
         /* reader wait */ 
...
void my_pthread_rwlock_wrlock(struct my_pthread_rwlock_t* rwlock) {
   ...
   /* only there is no reader and writer running can we let writer run*/ 
   if(rwlock->write_run == 0 && rwlock->read_run == 0) {
      rwlock->write_run = rwlock->write_run + 1;
   }
      
   /* read write mutex, write write mutex */ 
   else {
...
void my_pthread_rwlock_unlock(struct my_pthread_rwlock_t* rwlock) {
   ...
   if(rwlock->read_run > 0) {
      /* reader done */ 
      rwlock->read_run = rwlock->read_run - 1;
      /* if there is no reader, need to notify the writer */ 
      if(rwlock->read_run == 0) {
         /* if writer is waiting, notify it */ 
...
struct my_pthread_rwlock_t    rwlock;
